...
public class Game {
	...
	public static void removePlayer(CPlayer cp) {
		...
		cp.getPlayer().sendMessage(Msgs.Game_You_Left_A_Game.getString("<arena>", cp.getArena().getName()));
		Arena arena = cp.getArena();
		for (Player ppl : arena.getPlayers())
			if (ppl != cp.getPlayer())
				ppl.sendMessage(Msgs.Game_They_Left_A_Game.getString("<player>", cp.getName(), "<arena>", cp.getArena().getName()));
